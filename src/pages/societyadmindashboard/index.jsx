    ...
    >
      ...
      {/* Sidebar */}
        <aside className={`col-span-12 md:col-span-3 lg:col-span-2 ${open ? "block" : "hidden md:block"} md:ml-[-1rem]`}>
          <div
            className="rounded-3xl p-3 sticky top-20 flex flex-col"
            style={{
              background: "white",
...
function OverviewPage({ setPage }) {
  ...
  const loadOverviewData = async () => {
    try {
      ...
      const [membersResponse, eventsResponse, postsResponse] = await Promise.all([
        getSocietyMembers(society.societyId),
        getSocietyEvents(society.societyId, { limit: 5 }),
        getSocietyPosts(society.societyId, { limit: 5 })
      ]);
      ...
      // Create activity feed from recent events and posts
      // First sort all events and posts by date to get the most recent
      const sortedEvents = [...events]
        .filter(e => e.createdAt) // Only include events with createdAt timestamp
  ...
  const formatTimeAgo = (dateString) => {
    ...
    const now = new Date();
    const diffMs = Math.abs(now - date); // Use absolute value to remove negative
  ...
  if (loading) {
    return <Card title="Overview"><div className="text-center py-8">Loading...</div></Card>;
  }
    ...
    <>
      <Card title="Overview" subtitle="Quick stats for your society.">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <button
            ...
            className="rounded-2xl p-4 text-left hover:opacity-90 transition-opacity cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          {upcomingEvents.length === 0 ? (
            <div className="text-center py-8 text-sm opacity-70">No upcoming events</div>
          ) : (
            <div className="space-y-3">
              {upcomingEvents.map((e) => (
                <div key={e.eventId} className="p-3 rounded-2xl flex items-center justify-between gap-3" style={{ background: colors.paper }}>
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs opacity-70">{formatEventTime(e.startsAt)} • {e.location || 'TBA'}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Pill>{e.rsvps || 0} going</Pill>
                    <button
                      onClick={() => setPage('events')}
        ...
        >
          {recentPosts.length === 0 ? (
            <div className="text-center py-8 text-sm opacity-70">No posts yet</div>
          ) : (
            <div className="space-y-3">
              {recentPosts.map((p) => (
                <div key={p.postId} className="p-3 rounded-2xl flex items-center justify-between gap-3" style={{ background: colors.paper }}>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium truncate">{p.content.substring(0, 50)}...</div>
                    <div className="text-xs opacity-70">{p.likes || 0} likes • {formatTimeAgo(p.createdAt)}</div>
                  </div>
                  <button
                    onClick={() => setPage('posts')}
                    className="rounded-xl px-3 py-1 text-sm hover:bg-mist/80 transition-colors flex items-center gap-1 flex-shrink-0"
                    style={{ background: colors.mist }}
                  >
                    View
                  </button>
      ...
      <Card title="Recent activity" subtitle="Latest actions across your society">
        {recentActivity.length === 0 ? (
          <div className="text-center py-8 text-sm opacity-70">No recent activity</div>
        ) : (
...
function SettingsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Get the current user's managed society and user data
        const [societyData, userData] = await Promise.all([
          getMySociety(),
          getCurrentUser()
        ]);

        setSocietyId(societyData.societyId);
        ...
        const initialData = {
          ...
          adminSurname: userData.lastName || "",
          email: userData.email || "",
          phoneNumber: userData.phoneNumber || "",
          universityNumber: userData.universityNumber || "",
  ...
  useEffect(() => {
    if (activeTab === "Profile") {
      ...
      const changed =
        formData.adminName !== originalFormData.adminName ||
        formData.adminSurname !== originalFormData.adminSurname ||
    ...
    try {
      if (tab === "Profile") {
        ...
        const updateData = {
          ...
          lastName: formData.adminSurname,
  ...
  if (loading) {
    return (
      <Card
        ...
        subtitle="Update your society profile and personal account information"
      >
        <div className="space-y-4 animate-pulse">
          <div className="h-12 rounded-2xl" style={{ background: colors.paper }}></div>
    ...
    >
      ...
      {activeTab === "Security" && (
        <div>
          ...
          <p className="text-sm text-gray-600 mb-6">Update your personal account information and security settings.</p>
            ...
            <div className="space-y-3">
              <div className="flex flex-col items-center gap-3 rounded-3xl p-4" style={{ background: colors.paper }}>
                <div
                  ...
                  aria-hidden="true"
                >
                  {`${formData.adminName?.[0] || ''}${formData.adminSurname?.[0] || ''}`.toUpperCase() || '??'}
                </div>
                <div className="text-center">
                  <div className="font-semibold">
                    {`${formData.adminName} ${formData.adminSurname}`.trim() || 'Admin User'}
                  </div>
                  {(formData.universityNumber || formData.campus) && (
                    <div className="text-xs opacity-70">
                      {[formData.universityNumber, formData.campus].filter(Boolean).join(" • ")}
                    </div>
                  )}
                  {formData.email && (
            ...
            <div className="space-y-4 md:col-span-2">
              <div className="rounded-2xl p-4" style={{ background: "white", border: `1px solid ${colors.mist}` }}>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {/* First Name */}
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: colors.plum }}>
                      First name
                    </label>
                    <input
                      name="adminName"
                      ...
                      className="w-full border rounded-2xl px-4 py-2 outline-none transition-colors text-sm"
                      style={{
                        borderColor: validationErrors.adminName ? '#ef4444' : colors.mist,
                        backgroundColor: 'white'
                      }}
                    />
                    {validationErrors.adminName && (
                      <p className="text-red-500 text-xs mt-1">{validationErrors.adminName}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border rounded-2xl px-4 py-2 outline-none transition-colors text-sm"
                      style={{
                        borderColor: validationErrors.adminSurname ? '#ef4444' : colors.mist,
                        backgroundColor: 'white'
                      }}
                    />
                    {validationErrors.adminSurname && (
                      <p className="text-red-500 text-xs mt-1">{validationErrors.adminSurname}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <input
                    ...
                    readOnly
                    disabled
                    className="w-full border rounded-2xl px-4 py-2 outline-none text-sm opacity-60 cursor-not-allowed"
                    style={{
                      borderColor: colors.mist,
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    style={{
                      background: (isSubmitting || !hasChanges) ? colors.mist : colors.plum,
                      color: (isSubmitting || !hasChanges) ? "#555" : "white",
...
const NotificationSender = () => {
  ...
  const [deliveryStatus, setDeliveryStatus] = useState(() => {
    ...
    if (savedLogs) {
      try {
        ...
        return { sent: 0, failed: 0, pending: 0, logs: [] };
      }
    }
    return { sent: 0, failed: 0, pending: 0, logs: [] };
  });
  ...
  useEffect(() => {
    if (deliveryStatus.logs.length > 0) {
      localStorage.setItem('notificationLogs', JSON.stringify(deliveryStatus.logs));
    }
  }, [deliveryStatus.logs]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load active members
        const membersData = await getSocietyMembers(society.societyId, 'active');
  ...
  const getEligibleRecipients = () => {
    ...
    // In the future, can add more audience filters
    // For now, "all" means all active members
  ...
  const sendNotificationsToMembers = async () => {
    ...
    try {
      // Get recipient IDs
      const recipientIds = eligibleRecipients.map(member => member.studentId);
      ...
      eligibleRecipients.forEach(member => {
        logs.push({
      ...
      setDeliveryStatus({
        sent: 0,
        failed: eligibleRecipients.length,
        pending: 0,
        logs: []
      });
    } finally {
  ...
  return (
    <Card
      title="Send Notifications to Society Members"
      subtitle="Communicate with society members via in-app notifications"
    >
      {/* Success Message */}
      <AnimatePresence>
        {showSuccess && (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-300 hover:bg-gray-50 cursor-pointer transition-colors">
                  ...
                  />
                  <div>
                    <p className="font-medium text-gray-900 text-sm">All Active Members</p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-1 focus:ring-mediumpur focus:border-mediumpur outline-none text-sm"
                  >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="w-full px-4 py-2 bg-mediumpur text-white rounded-lg hover:bg-mediumpur/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium"
              >
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {deliveryStatus.logs.length > 0 && (
            <span className="text-xs text-gray-500">{deliveryStatus.logs.length} total sent</span>
          )}
        </div>
        ...
        {deliveryStatus.logs.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-400 mt-1">Your sent notifications will appear here</p>
          </div>
        ) : (
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {deliveryStatus.logs.map((log) => (
              <div
                key={log.id}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  {log.status === "failed" && (
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 text-xs rounded-lg">
                      <X className="w-3 h-3" />
                      Failed
                    </span>
                  )}
...
function MetricsPage() {
  ...
  const [membershipTrend, setMembershipTrend] = useState({
    labels: [],
    values: []
  });
  ...
  const loadMetrics = async () => {
    try {
      ...
      const [membersResponse, eventsResponse, postsResponse] = await Promise.all([
        getSocietyMembers(society.societyId),
        getSocietyEvents(society.societyId),
        getSocietyPosts(society.societyId)
      ]);

      // Extract data from responses
      // Members returns array directly, events/posts return {data, page, limit, total}
      const members = Array.isArray(membersResponse) ? membersResponse : (membersResponse?.data || []);
      const events = Array.isArray(eventsResponse) ? eventsResponse : (eventsResponse?.data || []);
      const posts = Array.isArray(postsResponse) ? postsResponse : (postsResponse?.data || []);
      ...
      const pendingCount = members.filter(m => m.status === 'pending').length;
      const now = new Date();
      const upcomingCount = events.filter(e => new Date(e.startsAt) > now).length;
      const totalLikes = posts.reduce((sum, post) => sum + (post.likes || 0), 0);

      // Calculate average event attendance (RSVPs)
      const avgAttendance = events.length > 0
        ? Math.round(events.reduce((sum, e) => sum + (e.rsvps || 0), 0) / events.length)
        : 0;
      ...
      // Get recent activity (last 5 events and posts combined)
      const recentEvents = events.slice(0, 3).map(e => ({
        type: 'event',
        title: e.title,
        date: e.startsAt,
        metric: `${e.rsvps || 0} RSVPs`
      }));
      ...
      const recentPosts = posts.slice(0, 3).map(p => ({
        ...
        date: p.createdAt,
        metric: `${p.likes || 0} likes`
      }));

      const combined = [...recentEvents, ...recentPosts]
      ...
      // Membership trend - calculate actual growth from join_date
      const last6Months = [];
      ...
      // Count cumulative members up to end of each month
      members.forEach(member => {
        const joinDate = new Date(member.joinDate);

        last6Months.forEach(monthData => {
          const monthEndDate = new Date(monthData.year, monthData.month + 1, 0); // Last day of month

          // If member joined before or during this month, count them
          if (joinDate <= monthEndDate) {
            monthData.count++;
          }
        });
      });
  ...
  if (isLoading) {
    return (
      <Card title="Society Metrics" subtitle="Comprehensive analytics dashboard">
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4" style={{ borderColor: colors.plum }}></div>
  ...
  return (
    <Card
      ...
      subtitle="Comprehensive analytics and insights for your society"
    >
      {/* Quick Stats Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="rounded-2xl p-4" style={{ background: colors.paper }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs font-medium px-2 py-1 rounded-full" style={{ background: colors.mist, color: colors.plum }}>
              +{stats.pendingRequests} pending
            </div>
          </div>
          <div className="text-2xl font-bold" style={{ color: colors.plum }}>{stats.totalMembers}</div>
        ...
        <div className="rounded-2xl p-4" style={{ background: colors.paper }}>
          <div className="flex items-center justify-between mb-2">
            <CalendarDays size={20} style={{ color: colors.plum }} />
            <div className="text-xs font-medium px-2 py-1 rounded-full" style={{ background: colors.mist, color: colors.plum }}>
              {stats.upcomingEvents} upcoming
            </div>
          </div>
          <div className="text-2xl font-bold" style={{ color: colors.plum }}>{stats.totalEvents}</div>
          <div className="text-sm text-gray-600">Total Events</div>
          <div className="text-xs text-gray-500 mt-1">Avg {stats.avgEventAttendance} RSVPs</div>
        </div>

        <div className="rounded-2xl p-4" style={{ background: colors.paper }}>
          <div className="flex items-center justify-between mb-2">
            <FileText size={20} style={{ color: colors.plum }} />
            <Heart size={16} style={{ color: colors.plum }} />
          </div>
          <div className="text-2xl font-bold" style={{ color: colors.plum }}>{stats.totalPosts}</div>
          <div className="text-sm text-gray-600">Total Posts</div>
          <div className="text-xs text-gray-500 mt-1">{stats.totalLikes} total likes</div>
        </div>

        <div className="rounded-2xl p-4" style={{ background: colors.paper }}>
          <div className="flex items-center justify-between mb-2">
            <BarChart3 size={20} style={{ color: colors.plum }} />
          </div>
          ...
          <div className="text-xs text-gray-500 mt-1">Likes per post</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-5" style={{ background: 'white', border: `1px solid ${colors.mist}` }}>
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.plum }}>Membership Growth</h3>
          <div className="space-y-3">
            {membershipTrend.labels.map((label, index) => (
              <div key={index} className="flex items-center gap-3">
                <div className="text-sm font-medium w-12 text-gray-600">{label}</div>
                <div className="flex-1">
                  <div className="h-8 rounded-xl relative overflow-hidden" style={{ background: colors.mist }}>
                    <div
                      className="h-full rounded-xl transition-all duration-500 flex items-center justify-end pr-3"
                      style={{
                        width: `${(membershipTrend.values[index] / Math.max(...membershipTrend.values)) * 100}%`,
                        background: colors.lilac
                      }}
                    >
                      <span className="text-xs font-medium text-white">{membershipTrend.values[index]}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Top Events */}
        <div className="rounded-2xl p-5" style={{ background: 'white', border: `1px solid ${colors.mist}` }}>
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.plum }}>Top Events by RSVPs</h3>
          <div className="space-y-3">
            {topEvents.length > 0 ? (
              topEvents.map((event, index) => (
                <div key={event.eventId} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: colors.paper }}>
                  <div
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold" style={{ color: colors.plum }}>{event.rsvps || 0}</div>
        ...
        <div className="lg:col-span-2 rounded-2xl p-5" style={{ background: 'white', border: `1px solid ${colors.mist}` }}>
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.plum }}>Recent Activity</h3>
          <div className="space-y-2">
            {recentActivity.length > 0 ? (
                ...
                <div key={index} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div
                      ...
                      style={{ background: item.type === 'event' ? colors.lilac : colors.mist }}
                    >
                      {item.type === 'event' ? (
                        <CalendarDays size={18} className="text-white" />
                      ) : (
                        <FileText size={18} style={{ color: colors.plum }} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate">{item.title}</div>
                      <div className="text-xs text-gray-500">
                        {new Date(item.date).toLocaleDateString()} • {item.metric}
                      </div>
                    </div>
                  </div>
                  <div className="text-xs px-2 py-1 rounded-full capitalize" style={{ background: colors.mist, color: colors.plum }}>
                    {item.type}
                  </div>
                </div>
              ))
      ...
      <div className="mt-6 flex justify-end">
        <button
          onClick={loadMetrics}
          className="flex items-center gap-2 px-4 py-2 rounded-2xl text-white transition-opacity hover:opacity-90"
          style={{ background: colors.plum }}
        >
