...
export default function LoginModal({ open, onClose, goSignup, goForgot }) {
  ...
  const handleLogin = (e) => {
    ...
    if (user) {
      ...
      setLoginStatus("error");
      setError("Incorrect university number or password. Please try again.");
      setShowSuccess(true);
        ...
        <Dialog.Panel className="relative w-full max-w-md rounded-2xl bg-white p-6 space-y-4 border border-white shadow-lg">
          ...
          {!showSuccess ? (
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <button
                type="submit"
                className="w-full rounded-lg bg-gradient-to-r from-mediumpur to-softlav py-3 text-white font-semibold shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200"
              >
                Log In
              </button>
            </form>
